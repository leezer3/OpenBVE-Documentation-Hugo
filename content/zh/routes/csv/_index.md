...
➟ [快速参考...]({{< ref "/routes/csv_quick/_index.md" >}}) 
...
- [2. 语法](#syntax)
- [3. 预处理](#preprocessing)
- [4. Options（选项）命名空间](#options)
- [5. Route（线路）命名空间](#route)
...
- [11. Track（轨道）命名空间](#track)
  ...
  - [11.3. 物体](#track_objects)
  - [11.4. 车站](#track_stations)
  - [11.5. 信号和限速](#track_signalling)
  - [11.6. 列车车载信号系统](#track_safety)
  - [11.7. 杂项](#track_misc)
...
线路文件是纯文本，且可以使用任意[字符编码]({{< ref "/information/encodings/_index.md" >}})，但是，我们推荐线路作者们使用UTF8-BOM编码格式。游戏在解析数字数据时，使用的[解析方法]({{< ref "/information/numberformats/_index.md" >}})是非常**宽松**的（特别指出的地方除外），但是，我们推荐线路作者们*严格*规范地编写线路文件。 这个文件一般来说可以被放在LegacyContent文件夹(或一个内包含*Railway*和*Train*目录的文件夹)下的任何地方。文件名可以随意，但扩展名必须是**.csv**。线路文件被从头到尾逐行解析，每行被分割并被从左到右解析。
...
➟ [还可以查阅这份CSV格式的快速参考手册。]({{< ref "/routes/csv_quick/_index.md" >}})

## <a name="syntax"></a>■ 2. 语法

对于线路文件中的每一行，在开头的[空格]({{< ref "/information/whitespaces/_index.md" >}})会被统统忽略。然后，每一行指令都会按逗号(U+002C，英文半角的那个)分割。于是，每一行都会被看作这样一个格式:

{{% command %}}
*表达式内容<sub>1</sub>*,* 表达式内容<sub>2</sub>*,* 表达式内容<sub>3</sub>*, ..., *表达式内容<sub>n</sub>*
{{% /command %}}
...
*一个干干净净清清爽爽的位置数字*
{{% /command %}}
一个非负的，[格式要正常]({{< ref "/information/numberformats/_index.md" >}})的浮点数(整数和小数)，代表一个主轨道位置。随后的指令都将按这个位置作为参考点。

{{% command %}}
*部分<sub>1</sub>*:*部分<sub>2</sub>*:...:*部分<sub>n</sub>*
{{% /command %}}
这是一个配合Options.UnitOfLength的更加复杂的距离指定格式。每一个*部分<sub>i</sub>*都是一个[格式要正常]({{< ref "/information/numberformats/_index.md" >}})的浮点数。*部分<sub>1</sub>*会被乘上*系数<sub>1</sub>*, *部分<sub>2</sub>*乘上*系数<sub>2</sub>*，以此类推，真正的主轨道位置是所有积的和。这个结果必须是非负的。各部分被冒号(U+003A，英文半角的那个)分隔。如果想了解更多请参见Options.UnitOfLength一节。

在任何参数中使用距离的命令中，这个冒号表示法就可以被使用，到时我们会用<font color="green">原谅色</font>标出这种情况。
...
{{% command %}}
*指令的大名* *参数<sub>1</sub>*;*参数<sub>2</sub>*;*参数<sub>3</sub>*;...;*参数<sub>n</sub>*
*指令的大名*(*参数<sub>1</sub>*;*参数<sub>2</sub>*;*参数<sub>3</sub>*;...;*参数<sub>n</sub>*)
{{% /command %}}
...
{{% command %}}  
*指令的大名*(*编号<sub>1</sub>*;*编号<sub>2</sub>*;...;*编号<sub>m</sub>*) *参数<sub>1</sub>*;*参数<sub>2</sub>*;*参数<sub>3</sub>*;...;*参数<sub>n</sub>*  
*指令的大名*(*编号<sub>1</sub>*;*编号<sub>2</sub>*;...;*编号<sub>m</sub>*).*后缀* *参数<sub>1</sub>*;*参数<sub>2</sub>*;*参数<sub>3</sub>*;...;*参数<sub>n</sub>*  
*指令的大名*(*编号<sub>1</sub>*;*编号<sub>2</sub>*;...;*编号<sub>m</sub>*).*后缀*(*参数<sub>1</sub>*;*参数<sub>2</sub>*;*参数<sub>3</sub>*;...;*参数<sub>n</sub>*)  
{{% /command %}}
