...
在游戏开始解析线路文件之前，预处理指令将会被预先处理替换。预处理器会按照正常顺序分析这些以$开头的预处理命令。$Chr、$Rnd和$Sub指令可以随便嵌套，$If、$Else和$EndIf不能出现在另一个指令的括号里。
...
该指令创建一个不可见的缓冲器，用来停住列车。列车可以从从前后双向撞上它。在线路的开始和结束处都放上这个指令，以防止玩家驾车出图，这就十分尴尬。该指令并不会放置可见的物体，所以如果必要，使用Track.FreeObj放置一个可见的实际代表物体。
